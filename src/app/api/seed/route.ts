...
export async function GET() {
    console.log('Début du seeding de la base de données via API...')
    ...
    try {
        ...
        return NextResponse.json({ message: 'Seeding terminé avec succès !' })
    } catch (error) {
        console.error(error)
        return NextResponse.json({ error: 'Erreur lors du seeding', details: error }, { status: 500 })
    }
}
