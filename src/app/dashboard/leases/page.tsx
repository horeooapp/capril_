                                    ...
                                    <tr key={lease.id} className="hover:bg-gray-50/30 transition-all group">
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black text-[#FF8200] mb-1">{lease.leaseReference}</span>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                                                        ...
                                                        lease.status === 'DRAFT' || lease.status === 'PENDING_SIGNATURE' ? 'bg-orange-50 text-orange-600' : 'bg-gray-50 text-gray-600'
                                                    }`}>
                                                        {lease.status === 'ACTIVE' ? 'Actif' : lease.status === 'DRAFT' || lease.status === 'PENDING_SIGNATURE' ? 'En attente' : lease.status}
                                                    </span>
                                                </div>
                                            </div>
                                        </td>
