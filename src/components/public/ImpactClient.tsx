...
export default function ImpactClient({ stats }: ImpactClientProps) {
  // Aggregate some metrics if data exists
  const totalSamples = stats?.reduce((acc: number, s: any) => acc + (s.sampleCount || 0), 0) || 0;
  const avgTrend = stats?.length > 0 
    ? (stats.reduce((acc: number, s: any) => acc + (s.trendPct || 0), 0) / stats.length).toFixed(1)
    : "0";
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-24 grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="glass-panel p-16 rounded-[3.5rem] border border-white shadow-2xl shadow-gray-200/50 hover:scale-[1.02] transition-transform duration-500 relative overflow-hidden group">
                        ...
                        <div className="text-primary font-black text-7xl mb-6 tracking-tighter italic">{avgTrend}%</div>
                        <h4 className="text-2xl font-black uppercase tracking-tight mb-6">Tendance des Loyers</h4>
                        <p className="text-gray-500 font-medium leading-relaxed text-lg">
                            Basé sur {totalSamples} contrats analysés au cours du dernier trimestre. Notre observatoire permet de détecter les variations du marché et d&apos;orienter les politiques de régulation.
                        </p>
                    </div>
                    <div className="glass-panel p-16 rounded-[3.5rem] border border-white shadow-2xl shadow-gray-200/50 hover:scale-[1.02] transition-transform duration-500 relative overflow-hidden group">
