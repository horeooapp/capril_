...
async function run() {
  try {
    ...
    let sql = 'INSERT INTO "public"."feature_flags" ("flag_id", "name", "description", "enabled", "updated_at") VALUES\n';
    sql += allFlags.map(f => `('${f.id}', '${f.name.replace(/'/g, "''")}', '${f.description.replace(/'/g, "''")}', ${f.enabled}, NOW())`).join(',\n');
    sql += '\nON CONFLICT ("flag_id") DO UPDATE SET "name" = EXCLUDED."name", "description" = EXCLUDED."description";';

    console.log('Executing Seed SQL via sudo -u postgres on VPS...');
